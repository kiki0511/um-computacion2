...
class ReadWriteLock:
    ...
    def acquire_read(self):
        """Adquirir lock para lectura. Espera si hay un escritor activo."""
        with self.lock:
    ...
    def release_read(self):
        """Liberar lock de lectura. Si era el último lector, notifica escritores."""
        with self.lock:
            self.readers -= 1
            if self.readers == 0:
    ...
    def acquire_write(self):
        """Adquirir lock para escritura. Espera si hay lectores o escritores."""
        with self.lock:
    ...
    def release_write(self):
        """Liberar lock de escritura. Notifica a lectores y al próximo escritor."""
        with self.lock:
            self.writers -= 1
